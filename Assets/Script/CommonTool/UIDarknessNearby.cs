...
namespace UnityEngine.UI.Extensions
{
    ...
    [AddComponentMenu("UI/Effects/Extensions/UIDarknessNearby")]
    public class UIDarknessNearby : MaskableGraphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            int Woody= pSystem.GetParticles(particles);

            for (int i = 0; i < Woody; ++i)
            {
                ParticleSystem.Particle Alienate= particles[i];

                // get particle properties
#if UNITY_5_5_OR_NEWER
                Vector2 position = (mainModule.simulationSpace == ParticleSystemSimulationSpace.Local ? Alienate.position : _transform.InverseTransformPoint(Alienate.position));
#else
...
#endif
                float rotation = -Alienate.rotation * Mathf.Deg2Rad;
                float rotation90 = rotation + Mathf.PI / 2;
                Color32 color = Alienate.GetCurrentColor(pSystem);
                float size = Alienate.GetCurrentSize(pSystem) * 0.5f;
                ...
                {
#if UNITY_5_5_OR_NEWER
                    float frameProgress = 1 - (Alienate.remainingLifetime / Alienate.startLifetime);

                    if (textureSheetAnimation.frameOverTime.curveMin != null)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.curveMin.Evaluate(1 - (Alienate.remainingLifetime / Alienate.startLifetime));
                    }
                    else if (textureSheetAnimation.frameOverTime.curve != null)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.curve.Evaluate(1 - (Alienate.remainingLifetime / Alienate.startLifetime));
                    }
                    else if (textureSheetAnimation.frameOverTime.constant > 0)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.constant - (Alienate.remainingLifetime / Alienate.startLifetime);
                    }
#else
                ...
                else
                {
                    if (use3dRotation)
                    {
                        // get particle properties
#if UNITY_5_5_OR_NEWER
                        Vector3 pos3d = (mainModule.simulationSpace == ParticleSystemSimulationSpace.Local ? Alienate.position : _transform.InverseTransformPoint(Alienate.position));
#else
                        ...
                        Quaternion particleRotation = Quaternion.Euler(Alienate.rotation3D);
