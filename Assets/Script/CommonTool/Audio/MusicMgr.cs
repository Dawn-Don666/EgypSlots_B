...
public class MusicMgr : MonoSingleton<MusicMgr>
{
    ...
    // 正在淡出的旧 BGM 源
    private AudioSource m_fadeOutSource;
    // 防止重复调用
    ...
    private void PlayBgBase(object bgName, bool restart = false)
    {
        ...
        // 1. 取目标 clip
        if (!AudioSettingDict.TryGetValue(BGM_Name, out AudioModel model))
        {
            Debug.LogError($"[MusicMgr] 找不到 BGM 配置 {BGM_Name}");
            return;
        }
        AudioClip targetClip = Resources.Load<AudioClip>(model.filePath);
        if (targetClip == null)
        {
            Debug.LogError($"[MusicMgr] 找不到音频资源 {model.filePath}");
            return;
        }

        float targetVolume = BgMusicSwitch ? getVolume(BGM_Name) : 0f;

        // 2. 同一首且不要求重播，直接返回
        if (!restart &&
            m_bgMusic != null &&
            m_bgMusic.clip == targetClip &&
            m_bgMusic.isPlaying)
            return;
        ...
        if (m_isFading)
        {
            DOTween.Kill(m_fadeOutSource);   // 停掉之前的 tween
            if (m_fadeOutSource)
            {
                m_fadeOutSource.Stop();
                AudioQueue.UnUseAudioComponent(m_fadeOutSource);
        ...
        if (m_bgMusic != null && m_bgMusic.isPlaying)
        {
            m_fadeOutSource = m_bgMusic;
            m_bgMusic = null;   // 置空，等旧源淡出完再创建新源
            m_isFading = true;

            // DOTween 淡出
            m_fadeOutSource.DOFade(0f, 0.2f)
                ...
                .OnComplete(() =>
                {
                    ...
                    // 旧源淡出完再播新源
                    PlayNewBg(targetClip, targetVolume);
                });
        }
        else
        {
    ...
    private void PlayNewBg(AudioClip clip, float volume)
    {
        m_bgMusic = AudioQueue.GetAudioComponent();
        m_bgMusic.loop = true;
        m_bgMusic.playOnAwake = false;
    ...
    public void StopBG()
    {
        ...
        {
            m_bgMusic.Stop();
            AudioQueue.UnUseAudioComponent(m_bgMusic);
            m_bgMusic = null;
        }
    ...
    // 保存“正在循环”的源
    private readonly Dictionary<MusicType.UIMusic, AudioSource> loopingSources = new();
    ...
    public void PlayLoop(MusicType.UIMusic effectName, bool defAudio = true, float volumeScale = 1f)
    {
        if (!EffectMusicSwitch) return;          // 总开关关闭直接返回

        StopLoop(effectName);                    // 先停旧实例

        /* 1. 拿池子里的源 */
        AudioSource source = AudioQueue.GetAudioComponent();
        source.loop = true;                      // 循环标识
        source.playOnAwake = false;

        /* 2. 加载音频 */
        string path = AudioSettingDict[effectName.ToString()].filePath;
        AudioClip clip = Resources.Load<AudioClip>(path);
        if (clip == null)
        {
            AudioQueue.UnUseAudioComponent(source); // 资源找不到，直接还回队列
            return;
        }
        source.clip = clip;

        /* 3. 音量计算：配置表 * 全局音效音量 * 外部倍数 */
        source.volume = getVolume(effectName.ToString()) * _EffectVolume * volumeScale;
        ...
        loopingSources[effectName] = source;
        PlayAudioSourceList.Add(source);        // 复用已有的定时回收逻辑
    }

    /// <summary>
    /// 停止指定循环音效并回收资源
    /// </summary>
    public void StopLoop(MusicType.UIMusic effectName)
    {
        if (!loopingSources.TryGetValue(effectName, out var src) || !src) return;

        src.Stop();                             // 立即停止
        AudioQueue.UnUseAudioComponent(src);    // 还回对象池
        PlayAudioSourceList.Remove(src);        // 从播放列表移除
        loopingSources.Remove(effectName);      // 注销字典
    }
