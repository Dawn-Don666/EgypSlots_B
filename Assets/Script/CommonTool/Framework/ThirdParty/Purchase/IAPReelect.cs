...
/// 内购接口实现类
/// </summary>
public class IAPReelect : IStoreListener
{
    public static IAPReelect Instance;
    ...
    public IAPReelect()
    {
        ...
        googlePublicKey = SinkElectroHerd.Instance.GooglePublicKey;
        appleRootCert = SinkElectroHerd.Instance.AppleRootCert;

        // 使用内购之前，先初始化Unity Gaming Services
        ConsequentUGS.Instance.Initialize();

        var module = StandardPurchasingModule.Instance();
        var builder = ConfigurationBuilder.Instance(module);

        List<Tube> shopList = TubeHerd.Instance.GetShopList(false);
        foreach(Tube shop in shopList)
        {
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs e)
    {
        ...
        try
        {
            ...
            foreach (IPurchaseReceipt productReceipt in result)
            {
                ...
                if (!TubeHerd.Instance.shopDict.ContainsKey(productId))
                {
        ...
        if (validPurchase)
        {
            ...
            TubeHerd.Instance.DistributeRewards(TubeHerd.Instance.GetShopById(shopId));
            purchaseCallback?.Invoke(true);
        }
        else
        {
            WharfReelect.GetInstance().ShowToast("Purchase failed");
            purchaseCallback?.Invoke(false);
        }
    ...
    public void StartPurchase(Tube shop, Action<bool> callBack)
    {
