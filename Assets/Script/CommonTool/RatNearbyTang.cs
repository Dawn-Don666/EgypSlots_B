...
{
    public static void Curly<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
...
public class RatNearbyTang : MonoYoungster<RatNearbyTang>
{
    // Start is called before the first frame update
    public bool PumpAlkali;
    public bool AnRise;

    static public RatNearbyTang instance;
    ...
    /// <returns></returns>
    public Vector3 AddDairySeven(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject RatDismissPestAdjust(Transform SelfTransform, List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float SevenOnForum(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Forum180On360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int RatRivalSupersede(Vector2 p1, Vector2 p2)
    {
        float angle = Forum180On360(SevenOnForum(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 AddThriftySoapCrop(Renderer Renderer)
    {
    ...
    public Vector2 AddThriftySoapCrop(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 AddImportCrop(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 AddIncurMetallic()
    {

        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = AddManageComet();
        float screenH = AddManageMonkey();
        float x = -screenW / 2 + xa * screenW;
        float y = -screenH / 2 + ya * screenH;
        return new Vector2(x, y);
    }

    public int AddAttackChimp(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] RatDisuseDonation(int total, int count)
    {
        ...
        {
            //随机一个数，每随机一次，随机区间-1
            int num = UnityEngine.Random.Range(0, Era + 1);
            output[i] = sequence[num];
            //将区间最后一个数赋值到取到的数上
            sequence[num] = sequence[Era];
            Era--;
        }
        return output;
    }
    public Dictionary<string, object> CosmosOnGeneration(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string GenerationOnCosmos(Dictionary<string, object> Info)
    {
    ...
    public int BowCoreDike(string dateStart, DateTime dateEnd)
    {
        DateTime start = Convert.ToDateTime(Convert.ToDateTime(dateStart).ToShortDateString());
        DateTime Era= Convert.ToDateTime(dateEnd.ToShortDateString());
        // 计算时间间隔
        TimeSpan sp = Era.Subtract(start);
        return sp.Days;
    }

    public int TedCoreDike(string dateStart, DateTime dateEnd)
    {
        DateTime start = Convert.ToDateTime(Convert.ToDateTime(dateStart));
        DateTime Era= dateEnd;
        // 计算时间间隔
        TimeSpan sp = Era.Subtract(start);
        return (int)sp.TotalSeconds;
    }
    public TimeSpan CoreDike(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 RatBitAtPoet(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = RatBitAtPoet(canvas, text, strFragmentIndex);
            Vector3 lastPos = RatBitAtPoet(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = RatBitAtPoet(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 RatBitAtPoet(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float PregnantByPole(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Neon= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string DivergeDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static string ExpanseDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Neon;
