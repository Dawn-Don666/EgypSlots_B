...
public class CoreDead
{
    public static long Slander()
    {
        return RatShoeshine(DateTime.Now);
    }

    public static long RatShoeshine(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string CoreTombBonito(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime EngraveOnCoreTomb(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string EngraveBonito(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string EngraveBonito2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
