...
public class UndertakeNeutrality : MonoBehaviour
{
    void Start()
    {
    ...
    public static void AidWith(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void AidBerg(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void MelodyCrease(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void MelodyCrease(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        MelodyCrease(startNum, endNum, delay, text, "", finish);
    }
    public static void MelodyCrease(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + CreaseDead.CenoteOnIon(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }
    ...
    public static void LifeCareSend(int num, Vector3 Start, Vector3 target, Transform parent = null, System.Action finish = null, bool isUpdate = false)
    {
        float Delaytime = 0;
        int a = 0;
        for (int i = 0; i < num; i++)
        {
            GameObject GoldItem = Instantiate(Resources.Load<GameObject>("Prefab/Leader"), parent);
    ...
    /// <param name="Finish"></param>
    public static void CongenitalRelate(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    /// <param name="finish"></param>
    public static void TrampleAim(Vector3 target, Vector3 Start, Transform parent, System.Action finish)
    {
