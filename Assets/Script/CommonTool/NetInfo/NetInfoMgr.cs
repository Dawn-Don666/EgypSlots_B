    ...
    void SetConfigData(string configJson)
    {
        ...
        if (ConfigData == null)
        {
            ...
            if (!string.IsNullOrEmpty(ConfigData.game_data))
            {
                ...
                GameDataManager.GetInstance().wildAddWeight = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).wildAddWeight;
                GameDataManager.GetInstance().maxLuckyWheelEnergy = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).maxLuckyWheelEnergy;
                GameDataManager.GetInstance().maxSpinCount = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).maxSpinCount;
                GameDataManager.GetInstance().specialRewardsItems = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).specialRewardsItems;
                GameDataManager.GetInstance().jackpotData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).jackpotData;
                GameDataManager.GetInstance().compareSizeData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).compareSizeData;
                GameDataManager.GetInstance().openBoxData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).openBoxData;
                GameDataManager.GetInstance().luckyWheelData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).luckyWheelData;
                GameDataManager.GetInstance().match3Data = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).match3Data;
                GameDataManager.GetInstance().scratchData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).scratchData;
                GameDataManager.GetInstance().luckyWheelData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).luckyWheelData;
                GameDataManager.GetInstance().goldPigData = JsonMapper.ToObject<GameConfig>(ConfigData.game_data).goldPigData;
            }

            //初始化插入格子
            if (!string.IsNullOrEmpty(ConfigData.specialTypeDict))
                GameDataManager.GetInstance().specialTypeDict = JsonMapper.ToObject<Dictionary<string, FixedSpecialTypeItem>>(ConfigData.specialTypeDict);
