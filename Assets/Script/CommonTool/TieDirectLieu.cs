...
{
    public static void Stair<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
...
public class TieDirectLieu : RestChristian<TieDirectLieu>
{
    // Start is called before the first frame update
    public bool GameDegree;
    public bool IfVest;

    static public TieDirectLieu instance;
    ...
    /// <returns></returns>
    public Vector3 PegBadlyGesso(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject TiePercentSinkBedpan(Transform SelfTransform, List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float GessoAnReply(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Reply180An360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int TieSenseBulldozer(Vector2 p1, Vector2 p2)
    {
        float angle = Reply180An360(GessoAnReply(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 PegCheaplyTrueBack(Renderer Renderer)
    {
    ...
    public Vector2 PegCheaplyTrueBack(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 PegSteadyBack(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 PegNeedyHardness()
    {

        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = PegCommitSuite();
        float screenH = PegCommitFrance();
        float x = -screenW / 2 + xa * screenW;
        float y = -screenH / 2 + ya * screenH;
        return new Vector2(x, y);
    }

    public int PegShrillShear(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] TieApieceStraight(int total, int count)
    {
        ...
        {
            //随机一个数，每随机一次，随机区间-1
            int num = UnityEngine.Random.Range(0, Arc + 1);
            output[i] = sequence[num];
            //将区间最后一个数赋值到取到的数上
            sequence[num] = sequence[Arc];
            Arc--;
        }
        return output;
    }
    public Dictionary<string, object> DemandAnSubglacial(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string SubglacialAnDemand(Dictionary<string, object> Info)
    {
    ...
    public int DonShoeHurt(string dateStart, DateTime dateEnd)
    {
        DateTime start = Convert.ToDateTime(Convert.ToDateTime(dateStart).ToShortDateString());
        DateTime Arc= Convert.ToDateTime(dateEnd.ToShortDateString());
        // 计算时间间隔
        TimeSpan sp = Arc.Subtract(start);
        return sp.Days;
    }

    public int DimShoeHurt(string dateStart, DateTime dateEnd)
    {
        DateTime start = Convert.ToDateTime(Convert.ToDateTime(dateStart));
        DateTime Arc= dateEnd;
        // 计算时间间隔
        TimeSpan sp = Arc.Subtract(start);
        return (int)sp.TotalSeconds;
    }
    public TimeSpan ShoeHurt(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 TieHayAtCrew(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = TieHayAtCrew(canvas, text, strFragmentIndex);
            Vector3 lastPos = TieHayAtCrew(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = TieHayAtCrew(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 TieHayAtCrew(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float HistoricByBury(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Tale= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string AmplifyDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static string SequoiaDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Tale;
