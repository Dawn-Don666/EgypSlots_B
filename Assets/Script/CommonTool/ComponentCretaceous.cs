...
public class ComponentCretaceous : MonoBehaviour
{
    void Start()
    {
    ...
    public static void HatSlow(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void HatFoul(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void LinearJewett(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void LinearJewett(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        LinearJewett(startNum, endNum, delay, text, "", finish);
    }
    public static void LinearJewett(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + JewettMesh.RanchoAnCup(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }
    ...
    public static void TileFirnHole(int num, Vector3 Start, Vector3 target, Transform parent = null, System.Action finish = null, bool isUpdate = false)
    {
        float Delaytime = 0;
        int a = 0;
        for (int i = 0; i < num; i++)
    ...
    /// <param name="Finish"></param>
    public static void HerbaceousMormon(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    /// <param name="finish"></param>
    public static void DespiteSty(Vector3 target, Vector3 Start, Transform parent, System.Action finish)
    {
