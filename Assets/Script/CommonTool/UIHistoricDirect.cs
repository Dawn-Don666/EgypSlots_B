...
namespace UnityEngine.UI.Extensions
{
    ...
    [AddComponentMenu("UI/Effects/Extensions/UIHistoricDirect")]
    public class UIHistoricDirect : MaskableGraphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            int Ocher= pSystem.GetParticles(particles);

            for (int i = 0; i < Ocher; ++i)
            {
                ParticleSystem.Particle Antelope= particles[i];

                // get particle properties
#if UNITY_5_5_OR_NEWER
                Vector2 position = (mainModule.simulationSpace == ParticleSystemSimulationSpace.Local ? Antelope.position : _transform.InverseTransformPoint(Antelope.position));
#else
...
#endif
                float rotation = -Antelope.rotation * Mathf.Deg2Rad;
                float rotation90 = rotation + Mathf.PI / 2;
                Color32 color = Antelope.GetCurrentColor(pSystem);
                float size = Antelope.GetCurrentSize(pSystem) * 0.5f;
                ...
                {
#if UNITY_5_5_OR_NEWER
                    float frameProgress = 1 - (Antelope.remainingLifetime / Antelope.startLifetime);

                    if (textureSheetAnimation.frameOverTime.curveMin != null)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.curveMin.Evaluate(1 - (Antelope.remainingLifetime / Antelope.startLifetime));
                    }
                    else if (textureSheetAnimation.frameOverTime.curve != null)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.curve.Evaluate(1 - (Antelope.remainingLifetime / Antelope.startLifetime));
                    }
                    else if (textureSheetAnimation.frameOverTime.constant > 0)
                    {
                        frameProgress = textureSheetAnimation.frameOverTime.constant - (Antelope.remainingLifetime / Antelope.startLifetime);
                    }
#else
                ...
                else
                {
                    if (use3dRotation)
                    {
                        // get particle properties
#if UNITY_5_5_OR_NEWER
                        Vector3 pos3d = (mainModule.simulationSpace == ParticleSystemSimulationSpace.Local ? Antelope.position : _transform.InverseTransformPoint(Antelope.position));
#else
                        ...
                        Quaternion particleRotation = Quaternion.Euler(Antelope.rotation3D);
