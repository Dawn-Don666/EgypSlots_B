...
public class TardinessHit : MonoBehaviour
{
    /* 字段 */
    private static TardinessHit _Ruminate;              //本脚本私有单例实例
    private Hashtable Ox= null;                        //容器键值对集合
    ...
    /// </summary>
    /// <returns></returns>
    public static TardinessHit RatRuminate()
    {
        if (_Ruminate == null)
        {
            _Ruminate = new GameObject("_ResourceMgr").AddComponent<TardinessHit>();
        }
        return _Ruminate;
    }
    ...
    /// <param name="isCatch"></param>
    /// <returns></returns>
    public GameObject FareTrick(string path, bool isCatch)
    {
