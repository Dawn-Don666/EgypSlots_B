...
public class SenseNorman : MonoBehaviour
{
    /// <summary>
    /// 查找子节点对象
    /// 内部使用递归
    ...
    /// <returns></returns>
    public static Transform FindDotCoachLife(GameObject goParent,string childName)
    {
        //查找结果
        ...
        if (searchTrans == null)
        {
            ...
            {
                searchTrans = FindDotCoachLife(trans.gameObject, childName);
                if (searchTrans != null)
                {
                    return searchTrans;
                }
    ...
    /// <returns></returns>
    public static T TieDotCoachLifeModernizeWhether<T>(GameObject goParent,string childName) where T:Component
    {
        //查找特定子节点
        Transform searchTranformNode = null;
        searchTranformNode = FindDotCoachLife(goParent, childName);
    ...
    /// <param name="childName">子对象名称</param>
    /// <returns></returns>
    public static T AgeCoachLifeModernize<T>(GameObject goParent,string childName) where T : Component
    {
        Transform searchTransform = null;
        searchTransform = FindDotCoachLife(goParent, childName);
    ...
    public static void AgeCoachLifeAnIntentLife(Transform parents,Transform child)
    {
